...
import { AlertTriangle, ShieldCheck, Activity, Bell, X } from "lucide-react";
...
export default function AlertsDashboard() {
  ...
  // New states for the alert pop-up
  const [showPopup, setShowPopup] = useState(false);
  const [latestAlert, setLatestAlert] = useState<BackendAlert | null>(null);
  ...
  const fetchAlerts = async () => {
    try {
      ...
      // Trigger pop-up if a new incident is detected (comparing timestamps)
      if (filteredData.length > 0) {
        const newest = filteredData[0];
        if (!latestAlert || newest.timestamp !== latestAlert.timestamp) {
          setLatestAlert(newest);
          setShowPopup(true);
          // Auto-hide pop-up after 6 seconds
  ...
  }, [latestAlert]); // Re-run effect when latestAlert updates to track new hits
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 dark:bg-black p-4 sm:p-6 md:p-8 transition-colors duration-300">
      {/* CATCHY POP-UP ALERT */}
      {showPopup && latestAlert && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-md animate-in fade-in zoom-in slide-in-from-top-4 duration-300">
          <div className="bg-red-600 dark:bg-red-500 text-white shadow-2xl rounded-3xl p-1 border-4 border-white dark:border-gray-900">
            ...
            <div className="px-5 pb-4">
              <p className="text-xs opacity-90 line-clamp-2 italic">
                "{latestAlert.why}"
              </p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        ...
        <section className="bg-white dark:bg-white/5 rounded-[2.5rem] p-6 border border-slate-200 dark:border-white/10 shadow-sm transition-all">
          ...
          <div className="w-full">
            {/* PASS THE LIVE DATA HERE */}
            <AlertsChart data={alerts} />
          </div>
        </section>
